...
requiredEnv.forEach((env) => {
  if (!process.env[env] || process.env[env].includes("your-")) {
    ...
    console.warn(
      "👉  Please update the Environment Variables in Render or your local .env file.\n",
    );
  }
});
...
const limiter = rateLimit({
  ...
  message: {
    success: false,
    message: "Rate limit exceeded. Too many requests. Please wait a minute.",
  },
...
app.get("/api/wake-up", (req, res) => {
  res
    .status(200)
    .json({ status: "online", message: "Server is awake and ready." });
});
...
async function sendToDiscord(messageData, systemInfo) {
  ...
  try {
    ...
    console.log(
      "✅ Discord notification sent successfully. Status:",
      response.status,
    );
  } catch (error) {
...
app.post("/api/contact", async (req, res) => {
  ...
  (async () => {
    try {
      ...
      console.log(
        `📧 Notification Protocol initialized using: ${process.env.EMAIL_USER}`,
      );
      ...
      const promises = [
        ...
        // User Auto-Reply
        axios
          .post("https://api.resend.com/emails", userAutoReplyPayload, {
            headers,
          })
